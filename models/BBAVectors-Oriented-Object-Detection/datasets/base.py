...
from .transforms import random_flip, load_affine_matrix, random_crop_info, ex_box_jaccard, random_crop_patch_info
...
class BaseDataset(data.Dataset):
    ...
    def data_transform(self, image, annotation):
        ...
        crop_size, crop_center = random_crop_patch_info(h=image.shape[0], w=image.shape[1], input_h=self.input_h,
    ...
    def generate_ground_truth(self, image, annotation, make_images: bool = False):
        image_org = np.asarray(np.clip(image, a_min=0., a_max=255.), np.float32)
        image = self.image_distort(np.asarray(image_org, np.float32))
        ...
        if make_images:
            try:
                copy_image1 = cv2.resize(image_org, dsize=(image_w, image_h))
            except Exception as e:
                print(f"image shape: {image_org.shape}")
                print(f"dsize: {(image_w, image_h)}")
                raise e
        ...
        for k in range(num_objs):
            ...
            # # draw
            if make_images:
                cv2.line(copy_image2, (int(cen_x), int(cen_y)), (int(cen_x), int(cen_y - wh[k, 9] / 2)), (0, 0, 255), 1,
                         1)
                cv2.line(copy_image2, (int(cen_x), int(cen_y)), (int(cen_x + wh[k, 8] / 2), int(cen_y)), (255, 0, 255),
                         1,
                         1)
                cv2.line(copy_image2, (int(cen_x), int(cen_y)), (int(cen_x), int(cen_y + wh[k, 9] / 2)), (0, 255, 255),
                         1,
                         1)
                cv2.line(copy_image2, (int(cen_x), int(cen_y)), (int(cen_x - wh[k, 8] / 2), int(cen_y)), (255, 0, 0), 1,
                         1)
            #####################################################################################
            # v0
        ...
        # ###################################### view Images #####################################
        if make_images:
            make_if_not_exist(f'/home/jmabon/tmp_data/bba_vec')
            copy_image1 = np.clip(copy_image1 / 255., 0.0, 1.0)
            copy_image2 = np.clip(copy_image2 / 255., 0.0, 1.0)
            file_id = np.random.randint(0, 10)
            try:
                plt.imsave(f'/home/jmabon/tmp_data/bba_vec/{file_id}-1.png', copy_image1)
            except ValueError as e:
                print('value error')
                print(f"{np.min(copy_image1), np.max(copy_image1)}")
                raise e
            try:
                plt.imsave(f'/home/jmabon/tmp_data/bba_vec/{file_id}-2.png', copy_image2)
            except ValueError as e:
                print('value error')
                print(f"{np.min(copy_image2), np.max(copy_image2)}")
                raise e
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'test':
            ...
            print(f'loading whole image {img_id}')
            image = self.processing_test(image, self.input_h, self.input_w)  # scales
            return {'image': image,
                    'img_id': img_id,
                    'image_w': image_w,
                    'image_h': image_h}
            ...
            image, annotation = self.data_transform(image, annotation)  # crops
            data_dict = self.generate_ground_truth(image, annotation, make_images=np.random.rand() < 0.05)
            return data_dict
